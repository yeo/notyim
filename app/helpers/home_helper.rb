...
module HomeHelper
  def new_slack_url
    if (current = Trinity::Current.current) && current.signed_in?
      "https://slack.com/oauth/authorize?state=#{current.user.id}.#{current.team.id}.#{SecureRandom.hex}&client_id=#{Rails.configuration.slack_bot[:client_id]}&scope=bot,incoming-webhook,chat:write:bot&redirect_uri=#{Rails.configuration.slack_bot[:redirect_uri]}"
    else
